...
import os 
import subprocess # for running the CUDA program to simulate parallelly
...
class NumericalPsystem():
    ...
    def runSimulationStep(self):
        ...
        for membraneName in self.H:
            ...
            if (type(membrane.chosenProgramNr) == int):
                logging.debug("Distribution for membrane %s of unitary value %f" % (
                    membraneName,
            ...
            # if this membrane uses enzymes (that allow multiple program execution)
                ...
                for i in range(len(membrane.chosenProgramNr)):
                    logging.debug("Distribution for membrane %s program %d of unitary value %f" % (
                        membraneName,
    ...
    def simulate(self, stepByStepConfirm = False, printEachSystemState = True, maxSteps = -1, maxTime = -1):
        ...
        logging.error("Simulation finished succesfully after %d steps and %f seconds; End state below:" % (currentStep, currentTime - startTime))
        self.print()
...
def extractSystem(system):
    """
    system : NumericalPSystem
    enzymes : list of string : stores the enzyme names
    prodFunction : string : stores the production functions appended together
    distFunction : string : stored the distribution functions appended together
    posProd : list of int : stores the ending index of the production function string (prodFunction)
    posDist : list of int : stores the ending index of the distribution function string (distFunction)
    """

    prodFunction = "" #stores the concatenated production function
    distFunction = "" #stores the concatenated distribution function
    newProdFunction = "" #used to calculate the length of production function with the new variable names
    newDistFunction = "" #used to calculate the length of distribution function with the new variable names
    numberOfPrograms = 0 #stores total number of programs in the given input
    numberOfVariables = [0] #stores cumulative number of variables in each membrane
    variables = [] #stores the values of the variables in the order encountered in the membranes
    enzymes = [] #stores information regarding enzymes in the memrbanes
    posProd = [0] #stores information regarding position of production function in string
    posDist = [0] #stores information regarding position of distribution function in string
    numberOfMembranes = len(system.H)
    variableDictionary = {} #mapping between variable names and assigned variable names
    membraneCounter = 0 #used in assigning new variable names
    membraneVariableList = [] #a list of all the variable and enzyme names in the input

    for m_name in system.H:
        membraneCounter += 1
        variableCounter = 0 
        membraneVariableList.append(m_name) #list - the membrane name is appended
        membrane = system.membranes[m_name]
        numberOfPrograms += len(membrane.programs) #int - number of programs in the file is updated
        numberOfVariables.append(len(membrane.variables) + len(membrane.enzymes)) #list - number of variables in the membranes - will be made cumulative later
        for var1 in membrane.variables:
            variableCounter += 1
            membraneVariableList.append(var1.name) #list - variable name is kept track of
            variables.append(var1.value)
            variableDictionary[var1.name] = '$_' + str(variableCounter) + '_' + str(membraneCounter) #dict - new name is given for variable
            
        membraneVariableList.append('$') # seperator between list of variables and enzymes
        for enz1 in membrane.enzymes:
            variableCounter += 1
            membraneVariableList.append(enz1.name) #list - enzyme name is kept track of
            variables.append(enz1.value)
            variableDictionary[enz1.name] = '$_' + str(variableCounter) + '_' + str(membraneCounter) #dict - new name is given for enzymes
    
    for m_name in system.H:
        membrane = system.membranes[m_name]
        for program in membrane.programs:
            newProdFunction = "(" + (program.prodFunction.infixExpression).replace(" ", "") + ")" #string - prod func without spaces
            newDistFunction = (program.distribFunction.expression).replace(" ", "") #string - dist func without spaces
            pf = program.prodFunction
            
            sortedPfList = []
            for item in pf.items:
                if(type(item)==Pobject):
                    sortedPfList.append(item.name) #list - names of production variables
            sortedPfList.sort(key=len, reverse=True) #list - same as above but sorted
            
            for item in sortedPfList:
                newProdFunction = newProdFunction.replace(item, variableDictionary[item]) #variable names are replaced in production functions

            df = program.distribFunction

            sortedDfList = []
            for item in df:
                sortedDfList.append(item.variable.name)
            sortedDfList.sort(key=len, reverse=True)
            for item in sortedDfList:
                newDistFunction = newDistFunction.replace(item, variableDictionary[item])

            posProd.append(len(newProdFunction) + len(prodFunction)) #list - position of prodFunc is updated
            posDist.append(len(newDistFunction) + len(distFunction)) #list - position of distFunc is updated
            prodFunction += newProdFunction #string - prodFunction is appended
            distFunction += newDistFunction #string - distFunction is appended
            if(type(program.enzyme) is Pobject): #list - membrane number and enzyme number of enzyme is added
                secondOccurence = variableDictionary[program.enzyme.name].find('_', 2)
                enzymes.append(variableDictionary[program.enzyme.name][2:secondOccurence]) #membrane number
                enzymes.append(variableDictionary[program.enzyme.name][secondOccurence+1:]) #enzyme number
            else:
                enzymes.append(-1)
                enzymes.append(-1)
    ...
    for pos in range(len(numberOfVariables)):
        if(pos!=0):
            numberOfVariables[pos] += numberOfVariables[pos-1] #list - number of variables in membranes - cumulative

    return [numberOfPrograms, numberOfMembranes, prodFunction, posProd, distFunction, posDist, numberOfVariables, variables, enzymes, membraneVariableList]

# end extractSystem(system)
...
if (__name__ == "__main__"):
    ...
    if("-p" in sys.argv):
        ...
        output_file = "cuda_input.txt"
        cuda_input = open(output_file, "w")


        cuda_input.write(str(parsed[0])+"\n")
        # numberOfPrograms

        cuda_input.write(str(parsed[1])+"\n")
        # numberOfMembranes

        cuda_input.write(str(len(parsed[2]))+" ")
        cuda_input.write(str(parsed[2])+"\n")
        # prodFunction

        cuda_input.write(str(len(parsed[3]))+" ")
        for i in parsed[3]:
            cuda_input.write(str(i)+" ")
        cuda_input.write("\n")
        # posProd

        cuda_input.write(str(len(parsed[4]))+" ")
        cuda_input.write(str(parsed[4])+"\n")
        # distFunction

        cuda_input.write(str(len(parsed[5]))+" ")
        for i in parsed[5]:
            cuda_input.write(str(i)+" ")
        cuda_input.write("\n")
        # posDist

        cuda_input.write(str(len(parsed[6]))+" ")
        for i in parsed[6]:
            cuda_input.write(str(i)+" ")
        cuda_input.write("\n")
        # numberOfVariables

        cuda_input.write(str(len(parsed[7]))+" ")
        for i in parsed[7]:
            cuda_input.write(str(i)+" ")
        cuda_input.write("\n")
        # variables
        ...
        os.system("nvcc enps.cu -o enps")
        os.system("./enps cuda_input.txt")
    else:
